...
@section('content-dashboard')
    <div class="bg-base-200 rounded-lg p-5">
        <h2 class="text-2xl font-bold mb-5">Add Car</h2>
        <form action="{{ route('cars.store') }}" method="post" class="flex flex-col gap-5" enctype="multipart/form-data">
            @csrf
                    ...
                    <select name="body_type_id" id="body_type_id" class="select" required>
                        @foreach ($body_types as $body_type)
                            <option value="{{ $body_type->id }}"
                                {{ old('body_type_id') === $body_type->id ? 'selected' : '' }}>
            ...
            <div class="form-control">
                ...
                <textarea id="description" type="text" placeholder="Description" name="description" class="textarea">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="form-control">
                <button type="submit" class="btn btn-primary w-fit">Add Car</button>
            </div>
        </form>
    </div>
@endsection
